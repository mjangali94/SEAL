...
{
    void DynArrayTest_DynArrayBasics(State &state)
    {
        for (auto _ : state)
        {
            state.PauseTiming();
            state.ResumeTiming();
            {
                ...
                seal::MemoryManager::SwitchProfile(new MMProfFixed(pool));
...
#ifdef SEAL_USE_MSGSL
    void DynArrayTest_FromSpan(State &state)
    {
        for (auto _ : state)
        {
            state.PauseTiming();
    ...
    void DynArrayTest_SaveLoadDynArray(State &state)
    {
        for (auto _ : state)
        {
            state.PauseTiming();
    ...
    void DynArrayTest_Assign(State &state)
    {
        for (auto _ : state)
        {
            state.PauseTiming();
            state.ResumeTiming();
            DynArray<char> arr(seal::MemoryManager::GetPool(mm_prof_opt::mm_force_new));
            DynArray<char> arr2(seal::MemoryManager::GetPool(mm_prof_opt::mm_force_new));
            ASSERT_NE(&static_cast<MemoryPool &>(arr.pool()), &static_cast<MemoryPool &>(arr2.pool()));

            arr = arr2;
            MemoryPool *addr = &static_cast<MemoryPool &>(arr.pool());
            ASSERT_EQ(&static_cast<MemoryPool &>(arr.pool()), addr);

            addr = &static_cast<MemoryPool &>(arr2.pool());
            arr = move(arr2);
            ASSERT_EQ(&static_cast<MemoryPool &>(arr.pool()), addr);
        }
    }
