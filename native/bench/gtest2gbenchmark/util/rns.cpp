...
namespace gtest2gbenchmark
{
    namespace util
    {
        void RNSBaseTest_Create(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void RNSBaseTest_ArrayAccess(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void RNSBaseTest_Copy(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
                state.ResumeTiming();
                auto pool = seal::MemoryManager::GetPool(mm_prof_opt::mm_force_new);
                ...
                ASSERT_EQ(2l, pool.use_count());
                {
                    RNSBase base2(base, seal::MemoryManager::GetPool());
                    ASSERT_EQ(2l, pool.use_count());
                    ASSERT_EQ(base.size(), base2.size());
                    ASSERT_EQ(base[0], base2[0]);
                    ASSERT_EQ(base[1], base2[1]);
        ...
        void RNSBaseTest_Contains(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
                state.ResumeTiming();
                RNSBase base({ 2, 3, 5, 13 }, seal::MemoryManager::GetPool());
        ...
        void RNSBaseTest_IsSubbaseOf(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
                state.ResumeTiming();
                {
                    RNSBase base({ 2 }, seal::MemoryManager::GetPool());
                    RNSBase base2({ 2 }, seal::MemoryManager::GetPool());
                ...
                {
                    RNSBase base({ 2 }, seal::MemoryManager::GetPool());
                    RNSBase base2({ 2, 3 }, seal::MemoryManager::GetPool());
                    ASSERT_TRUE(base.is_subbase_of(base2));
                ...
                // Order does not matter for subbase/superbase
                    RNSBase base({ 3, 13, 7 }, seal::MemoryManager::GetPool());
                    RNSBase base2({ 2, 3, 5, 7, 13, 19 }, seal::MemoryManager::GetPool());
                    ASSERT_TRUE(base.is_subbase_of(base2));
                    ASSERT_FALSE(base2.is_subbase_of(base));
                    ASSERT_FALSE(base.is_superbase_of(base2));
                    ASSERT_TRUE(base2.is_superbase_of(base));
                ...
                {
                    RNSBase base({ 3, 13, 7, 23 }, seal::MemoryManager::GetPool());
                    RNSBase base2({ 2, 3, 5, 7, 13, 19 }, seal::MemoryManager::GetPool());
        ...
        void RNSBaseTest_Extend(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void RNSBaseTest_Drop(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void RNSBaseTest_ComposeDecompose(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
                state.ResumeTiming();
                MemoryPoolHandle pool = seal::MemoryManager::GetPool();
        ...
        void RNSBaseTest_ComposeDecomposeArray(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
            ...
            void BaseConverterTest_Initialize(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void BaseConverterTest_Convert(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void BaseConverterTest_ConvertArray(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_Initialize(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_FastBConvMTilde(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_MontgomeryReduction(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_FastFloor(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_FastBConvSK(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_ExactScaleAndRound(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_DivideAndRoundQLastInplace(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
            ...
            void RNSToolTest_DivideAndRoundQLastNTTInplace(State &state)
            {
                for (auto _ : state)
                {
                    state.PauseTiming();
