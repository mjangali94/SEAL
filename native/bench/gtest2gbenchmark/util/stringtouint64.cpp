...
namespace gtest2gbenchmark
{
    namespace util
    {
        void StringToUInt64_IsHexCharTest(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void StringToUInt64_HexToNibbleTest(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void StringToUInt64_GetHexStringBitCount(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
        ...
        void StringToUInt64_HexStringToUInt64(State &state)
        {
            for (auto _ : state)
            {
                state.PauseTiming();
